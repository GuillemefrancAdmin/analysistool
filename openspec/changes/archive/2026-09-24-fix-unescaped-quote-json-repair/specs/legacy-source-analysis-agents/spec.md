...
The pipeline SHALL repair known, narrow classes of non-JSON artifacts in architecture-spec-writer's raw output — markdown code fences and prose preface, unescaped backslashes in echoed source snippets, unescaped double-quotes embedded inside a string value, and missing trailing closing braces/brackets — before attempting to parse the output as JSON, rather than treating any one of these as an immediate, unrecoverable parse failure.
...
- **WHEN** architecture-spec-writer's raw output is already well-formed JSON with no embedded-quote artifacts
