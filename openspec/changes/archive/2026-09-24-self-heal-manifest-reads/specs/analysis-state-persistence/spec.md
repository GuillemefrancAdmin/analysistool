...
`.analysis-state/queue/manifest.json` SHALL be the single mutable index of all tracked files and their status, safe for concurrent read by reporting tools (`queue_eta.ps1`) and concurrent read-modify-write by multiple pipeline workers, via the write safety and locking behavior defined in `source-discovery-queue` and `sequential-pipeline-execution`. Readers of the manifest SHALL additionally attempt to auto-repair a known, narrow corruption pattern — a small number of non-ASCII characters appearing in raw JSON structural whitespace, never inside a string value — before falling back to their existing retry/failure behavior, since the manifest's own content is always plain ASCII and any such character is reliably corruption rather than legitimate data.
...
- **WHEN** a manifest read fails to parse as JSON, and the raw content contains more than five characters outside the normal ASCII range, or the repaired content still fails to parse
